        ...
        public static async Task<List<Alliance>> GetRandomAlliancesAsync()
        {
            ...
            try
            {
                using (var connection = new MySqlConnection(_connectionString))
                {
                    ...
                    using (var cmd = new MySqlCommand($"SELECT * FROM {Name} ORDER BY RAND() LIMIT 10000",
                        connection))
                    {
                        var reader = await cmd.ExecuteReaderAsync();
        ...
        public static async Task<List<Alliance>> GetAllianceByNameAsync(string AllianceName)
        {
            ...
            try
            {
                using (var connection = new MySqlConnection(_connectionString))
                {
                    ...
                    {
                        using (var cmd = new MySqlCommand($"SELECT * FROM {Name} WHERE Id = '{Utilities.Utils.GameUtils.GetIDFromHashTag(AllianceName)}' OR Name = '{AllianceName}' ORDER BY RAND() LIMIT 1",
                            connection))
                        {
                            var reader = await cmd.ExecuteReaderAsync();
                    ...
                    {
                        using (var cmd = new MySqlCommand($"SELECT * FROM {Name} WHERE Name = '{AllianceName}' ORDER BY RAND() LIMIT 10000",
                            connection))
                        {
                            var reader = await cmd.ExecuteReaderAsync();
